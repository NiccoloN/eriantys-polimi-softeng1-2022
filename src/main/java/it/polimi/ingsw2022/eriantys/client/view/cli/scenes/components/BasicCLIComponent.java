...
public class BasicCLIComponent implements CLIComponent {
    ...
    protected void setRow(int index, String row) {
        ...
        if(length != width) throw new InvalidParameterException("Row must be of length " + width + ", like declared");
    }
        ...
        if(!hidden) {
            ...
            for(int i = 0; i < height; i++) {
                ...
                for(int j = 0; j < width; j++) {
                    ...
                    if(!chars[i][j].contains("\0") && y + i >= 0 && y + i < frame.getHeight() && x + j >= 0 && x + j < frame.getWidth()) frame.setChar(getFrameX() + j, getFrameY() + i, chars[i][j]);
                }
            }
        }
    }
