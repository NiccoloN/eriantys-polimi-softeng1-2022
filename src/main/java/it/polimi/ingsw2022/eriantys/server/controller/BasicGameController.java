...
public class BasicGameController implements GameController, Serializable {
    ...
    protected void playTurn(Player player, int playerIndex) throws IOException, InterruptedException {
        ...
        if(playerIndex < game.getPlayers().size() - 1) server.sendToClient(new MoveRequestMessage(new WaitRequest()), player.getUsername());
    }
    ...
    protected void requestStudent(Player player) throws IOException, InterruptedException {
        ...
        requestMove(new MoveStudentRequest(player.getSchool().getAvailableEntranceColors(), List.of(ColoredPawnOriginDestination.ISLAND, ColoredPawnOriginDestination.TABLE), !player.hasPlayedCharacter()), player.getUsername());
    }
